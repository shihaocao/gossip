...
def test_update_node():
    ...
    assert {'123': {456: DataPoint(now, 2), 457: DataPoint(
        now+1, 3)}} == dict(test_state.ip_map)
    test_state.update_node('123', 458, now+2, 4)
    assert {'123': {456: DataPoint(now, 2), 457: DataPoint(
        now+1, 3), 458: DataPoint(now+2, 4)}} == dict(test_state.ip_map)

    # test only keeping the three newest ports
    test_state.update_node('123', 459, now+3, 5)
    assert {'123': {459: DataPoint(
        now+3, 5), 457: DataPoint(now+1, 3), 458: DataPoint(now+2, 4)}} == dict(test_state.ip_map)
    ...
    assert {'123.10': {42: DataPoint(now, 1)}, '123': {457: DataPoint(
        now+1, 3), 458: DataPoint(now+2, 4)}} == dict(test_state.ip_map)
...
def test_get_state():
    ...
    result_list = test_state.get_state(printable=False)
